...
export function Plan({ data, label, isCurrentPlan, index, activeTab, monthlyPlans }: { data?: any, label?: string; isCurrentPlan?: boolean; index?: number, activeTab: "monthly" | "yearly", monthlyPlans: any[] }) {
    ...
    return (
        <>
            ...
            >
                ...
                <div>
                    ...
                    <h3 className="text-[#2E384E] dark:text-white text-[28px] font-medium mt-[8px] whitespace-nowrap">
                        {data.name !== "Custom" ? activeTab === "yearly" ? (
                            <>
                                {data.price === 0 ? <span className="text-[18px]">Free</span> : (<>
                ...
                <div
                    className={`group relative w-fit flex items-center ${label && "w-full justify-center"} gap-[8px] px-[30px] py-[15px] rounded-[8px] ${getButtonColor(data.name)} ${isCurrentPlan && "bg-white border border-[#FFA500] text-[#2E384E] cursor-not-allowed"}`}>
                    ...
                    <p
                        ...
                        className={`text-[#FFF] cursor-pointer ${isCurrentPlan && "text-black"}`}>
                        {label ? label : `Choose ${data.name} plan`}
                    </p>
